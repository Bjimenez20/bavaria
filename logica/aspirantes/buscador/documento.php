...
if (empty($document)) {
    ...
    if (empty($document)) {
        array_push($campos_vacios, 'Numero de documento');
    }
    ...
    if ($resultado && mysqli_num_rows($resultado) > 0) {
        ...
        $response = array(
            'mensaje' => 'No hemos podidos encontrar el documento, por favor intenta con el telefono',
            'tipo' => 'warning',
            'title' => 'Habilitando campo de telefono...',
            'validation' => 'document_error'
        );
        echo json_encode($response);
    }
}
