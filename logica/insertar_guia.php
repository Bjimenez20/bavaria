...
require('../datos/parse_str.php');
	...
	if (isset($valor)) {
		...
		$sql = mysqli_query($conex,"UPDATE `bayer_movimientos` SET
		ESTADO_MOVIMIENTO='ENTREGADO'
		WHERE ID_MOVIMIENTOS = '" . $ID_MOVIMIENTO . "';");
		echo mysqli_error($conex);
		$actualizar_movimiento_pac = mysqli_query($conex,"UPDATE `bayer_paciente_movimientos` SET 		   
	...
	if (isset($_POST['insertar'])) {
		for ($i = 1; $i <= $x; $i++) {
			if (isset($_POST["id" . $i])) {
				...
				if ($NO_GUIA != '' && $SERIAL != '') {
					$consul = mysqli_query($conex,"SELECT * FROM bayer_inventario WHERE CODIGO_PRODUCTO='$SERIAL' AND LUGAR_MATERIAL='BODEGA' AND ID_REFERENCIA_FK='$ID_REFERENCIA'");
					...
					if ($num_total > 0) {
						...
						$update_inv = mysqli_query($conex,"UPDATE `bayer_inventario` SET 		
