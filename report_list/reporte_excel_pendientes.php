...
if (isset($_POST['exportar'])) {
	...
	if ($TIPO_SOLICITUD == 'PENDIENTES') {
		if ($fecha_ini != ' 00:00:00' || $fecha_fin != ' 23:59:59') {
			$consulta_paciente = mysqli_query($conex, "SELECT * FROM ipsen_movimientos AS M INNER JOIN ipsen_referencia AS R ON R.ID_REFERENCIA=M.ID_REFERENCIA_FK INNER JOIN ipsen_paciente_movimientos AS PM ON PM.ID_MOVIMIENTOS_FK=M.ID_MOVIMIENTOS
	...
	if ($TIPO_SOLICITUD == 'ENTREGADO') {
		$consulta_paciente = mysqli_query($conex, "SELECT * FROM ipsen_inventario AS I
