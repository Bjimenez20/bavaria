...
class Informe_reclamacion_historial_total
{
   ...
   //----- 
   function resumo_sc_free_group_by($destino_resumo, &$array_total_bayer_pacientes_id_paciente, &$array_total_bayer_pacientes_estado_paciente)
   {
      global $nada, $nm_lang;
   ...
   foreach ($all_sql_free as $cmp_gb)
   {
      ...
      { 
         $comando  = "select count(*), bayer_pacientes.ID_PACIENTE, bayer_pacientes.ESTADO_PACIENTE from " . $this->Ini->nm_tabela . " " .  $this->sc_where_atual . " group by " . $free_group_by . "  " . $nmgp_order_by;
      } 
