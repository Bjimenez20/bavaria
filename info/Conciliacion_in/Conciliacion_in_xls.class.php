...
class Conciliacion_in_xls
{
   ...
   function grava_arquivo()
   {
      ...
      while (!$rs->EOF)
      {
         $this->xls_col = 0;
         $this->xls_row++;
         $this->bayer_pacientes_fecha_activacion_paciente = $rs->fields[0] ;  
         $this->bayer_pacientes_nombre_paciente = $rs->fields[1] ;  
         $this->bayer_pacientes_apellido_paciente = $rs->fields[2] ;  
         $this->bayer_pacientes_telefono_paciente = $rs->fields[3] ;  
         $this->bayer_pacientes_telefono2_paciente = $rs->fields[4] ;  
         $this->bayer_pacientes_telefono3_paciente = $rs->fields[5] ;  
         $this->bayer_tratamiento_producto_tratamiento = $rs->fields[6] ;  
          if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_informix))
          { 
              $this->bayer_gestiones_descripcion_comunicacion_gestion = "";  
              if (is_file($rs_grid->fields[7])) 
              { 
                  $this->bayer_gestiones_descripcion_comunicacion_gestion = file_get_contents($rs_grid->fields[7]);  
              } 
          } 
         elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase))
         { 
             $this->bayer_gestiones_descripcion_comunicacion_gestion = $this->Db->BlobDecode($rs->fields[7]) ;  
         } 
         else
         { 
             $this->bayer_gestiones_descripcion_comunicacion_gestion = $rs->fields[7] ;  
         } 
         $this->bayer_gestiones_autor_gestion = $rs->fields[8] ;  
         $this->bayer_pacientes_id_paciente = $rs->fields[9] ;  
   ...
   function NM_export_bayer_pacientes_telefono3_paciente()
   {
         ...
         $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->xls_col) . $this->xls_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
         $this->Nm_ActiveSheet->setCellValueExplicit($this->calc_cell($this->xls_col) . $this->xls_row, $this->bayer_pacientes_telefono3_paciente, PHPExcel_Cell_DataType::TYPE_STRING);
         $this->xls_col++;
   }
