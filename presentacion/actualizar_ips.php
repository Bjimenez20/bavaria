...
<body>
    <section>
        <blockquote>
            <?php
            $ips = mysqli_query($conex, "SELECT * FROM ipsen_ips WHERE ID_IPS = '" . $ID . "'");
            while ($fila = mysqli_fetch_array($ips)) {
            ?>
                <form action="editar_ips.php?erted=<?php echo $fila['ID_IPS'] ?>" method="POST">
                    <table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" style="margin:auto auto;">
                        <tr>
                            <th colspan="4">
                                <strong>HABILITAR IPS</strong>
                            </th>
                        </tr>
                        <tr>
                            <td>
                                <span>IPS POR HABILITAR</span>
                            </td>
                            <td>
                                <input type="text" name="ips_habilitar" id="ips_habilitar" value="<?php echo $fila['IPS'] ?>">
                            </td>
                        </tr>
                        <tr>
                            <th colspan="4">
