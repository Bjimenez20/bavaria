...
<body>
    <section>
        <blockquote>
            <?php
            $operador = mysqli_query($conex, "SELECT * FROM ipsen_operador_logistico WHERE ID_OPERADOR_LOGISTICO = '" . $ID . "'");
            while ($fila = mysqli_fetch_array($operador)) {
            ?>
                <form action="editar_operador.php?ertid=<?php echo $ID ?>" method="POST">
                    <table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" style="margin:auto auto;">
                        <tr>
                            <th colspan="4">
                                <strong>HABILITAR OPERADOR</strong>
                            </th>
                        </tr>
                        <tr>
                            <td>
                                <span>OPERADOR LOGISTICO POR HABILITAR</span>
                            </td>
                            <td>
                                <input type="text" name="operador_habilitar" id="operador_habilitar" value="<?php echo $fila['OPERADOR_LOGISTICO'] ?>">
                            </td>
                        </tr>
                        <tr>
                            <th colspan="4">
