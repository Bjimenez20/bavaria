...
$fecha = date("Y-m-d");
$select_id = mysqli_query($conex, "SELECT ID_GESTION, ID_PACIENTE, CLASIFICACION_PATOLOGICA_TRATAMIENTO, ASEGURADOR_TRATAMIENTO, OPERADOR_LOGISTICO_TRATAMIENTO, PUNTO_ENTREGA, CIUDAD_PACIENTE,PRODUCTO_TRATAMIENTO, 
DOSIS_TRATAMIENTO,CAUSA_NO_RECLAMACION_GESTION, MEDICO_TRATAMIENTO, NUMERO_PENDIENTE, FECHA_PENDIENTE, AUTOR_GESTION, ASIGNADO_EDUGESTOR, OBSERVACION_ESCALAMIENTO FROM ipsen_pacientes INNER JOIN ipsen_gestiones ON ipsen_pacientes.ID_PACIENTE = ipsen_gestiones.ID_PACIENTE_FK2 INNER JOIN ipsen_tratamiento ON ipsen_pacientes.ID_PACIENTE = ipsen_tratamiento.ID_PACIENTE_FK WHERE ID_GESTION = '$id_gestion' ORDER BY FECHA_COMUNICACION DESC LIMIT 1");
